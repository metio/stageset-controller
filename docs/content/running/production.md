...
and how restore selects a revision are covered in [Rollback](/gating/rollback/).
...
[encryption at rest](/gating/rollback/#encryption-at-rest). This is distinct from
...
[multi-cluster and tenancy](/security/multi-cluster/). To hard-isolate namespaces,
...
[Scale and capacity](/running/scale-and-capacity/) covers the sizing drivers
...
a stage runs `http` [actions](/defining-a-release/actions/), the hosts those actions may reach
...
[Operations](/running/operations/#metrics).
...
- [Configuration reference](/reference/configuration/) — every flag, its
  default, and the Helm value that drives it.
- [Operations](/running/operations/) — metrics, alerts, events, forcing a
  reconcile, drift correction.
- [Rollback](/gating/rollback/) — store backends, keep-count, and revision
  selection.
- [Multi-cluster and tenancy](/security/multi-cluster/) — tenant ServiceAccounts,
