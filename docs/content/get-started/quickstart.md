...
- [Stage sources](/get-started/flux-sources/) — direct `GitRepository`,
  `OCIRepository`, and `Bucket` sources, plus the renderer route.
- [Usage](/defining-a-release/) — every configuration knob, one page per concern.
- [Installation](/get-started/) — production-grade install: HA, rollback store,
