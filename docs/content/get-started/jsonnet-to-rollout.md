...
[Stage sources](/get-started/flux-sources/). The renderer is here because the input is
...
- StageSet installed (see [Installation](/get-started/kubernetes/)).
- An `apps` namespace, and a `web-deployer` `ServiceAccount` in it whose RBAC can
  ...
  # Services in the apps namespace — see /security/multi-cluster/ for the tenancy model
  ```
...
To gate one-time [migrations](/gating/versioned-migrations/) on a release boundary,
...
[`version.fromArtifact`](/gating/versioned-migrations/#from-a-file-in-the-artifact--versionfromartifact)
instead. See [versioned migrations](/gating/versioned-migrations/) for all three.

## Next

From here, add more [stages](/defining-a-release/stages-and-sources/), pre/post
[actions](/defining-a-release/actions/), or [update windows](/gating/update-windows/) to turn
this single rollout into a gated, multi-stage release. To parameterize per
environment, see [Parameters](/guides/parameters/).
