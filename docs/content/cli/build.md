...
| `--as-tenant` | `false` | Render each stage impersonating its effective `serviceAccountName` — the stage's own, else the StageSet's `spec.serviceAccountName` (see [multi-cluster and tenancy](/security/multi-cluster/)). |
