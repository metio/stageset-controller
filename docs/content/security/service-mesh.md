...
Where the [network policy](/security/network-policy/) operates at L3/L4 — which pods and
...
The mesh-reachable ports are the gate port and the metrics port:

| Port | Mode | `from` restricts |
|---|---|---|
| Gate (`ports.gate`, `8082`) | gate | Flagger polling the read-only gate endpoint |
...
|---|---|---|
| Gate (`8082`) | Yes — `serviceMesh.gate.from` | Meshed Flagger polling the gate endpoint |
