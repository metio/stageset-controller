...
- **One operation per action** — every entry in `actions.pre`/`post`/`onFailure`
  ...
  `job` / `delete` / `apply`. Zero or two is rejected. See [actions](/defining-a-release/actions/).
- **One version source** — when `spec.version` is set, exactly one of `value`,
  `fromObject`, or `fromArtifact` (see [versioned migrations](/gating/versioned-migrations/)).
...
The [configuration reference](/reference/configuration/) lists the full
`--webhook-*` flag set.
