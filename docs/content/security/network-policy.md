...
| Gate (`ports.gate`, `8082`) | Flagger, polling the read-only gate endpoint for the promotion verdict | gate | Yes — by Flagger's namespace |
...
[multi-cluster](/security/multi-cluster/), [rollback](/gating/rollback/),
[producer-aware sources](/integrations/producer-aware-sources/), and
[actions](/defining-a-release/actions/) for the flows behind each row.
