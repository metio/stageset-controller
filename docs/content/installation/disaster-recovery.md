...
leader lease are all reconstructed automatically. Three pieces of state need
backup attention — two are unrecoverable if lost, and the third self-heals for the
common case but is still worth backing up:
...
2. **Encryption key Secrets** — the age or PGP private keys named by
   ...
   [rollback](/usage/rollback/) that must re-decrypt a prior revision fails closed
...
3. **Rollback store contents** — when `spec.rollbackOnFailure: true` and the
   producer retains only one revision, the [rollback store](/usage/rollback/) (a
...
1. **Install the controller** from the Helm chart. See
   [Production](/installation/production/) for the hardened install.
...
after step 6, you remove the race entirely.
...
- [Inventory and pruning](/usage/inventory/) — what the `StageInventory` records and how the prune diff works.
