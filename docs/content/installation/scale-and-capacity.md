...
[Production](/installation/production/#high-availability) for the resource block in
context.
...
[rollback store](/usage/rollback/) must be reachable from whichever pod holds the
...
[Production](/installation/production/#high-availability) page has the full HA
...
[Inventory and pruning](/usage/inventory/).
...
- [Production](/installation/production/#high-availability) — the hardened install, HA replicas, and the resource block in context.
- [Inventory and pruning](/usage/inventory/) — sharding and the `StageInventory` the shard cap governs.
- [Multi-cluster and tenancy](/usage/multi-cluster/) — watch-scoped instances, the leader model, and cross-namespace isolation.
...
- [Backup and disaster recovery](/installation/disaster-recovery/) — the state a multi-replica install must keep reachable.
