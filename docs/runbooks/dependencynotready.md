...
Resolve the dependency's own Ready condition first (follow its runbook). Once it reports `Ready=True` at its current generation, this StageSet proceeds on the next reconcile. If the dependency is intentionally suspended, this StageSet will wait indefinitely by design — remove the `dependsOn` entry or resume the dependency.
